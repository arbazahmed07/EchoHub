...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Check for GitHub connection result in URL params
    const urlParams = new URLSearchParams(window.location.search);
    const githubResult = urlParams.get('github');
  ...
  const checkGithubStatus = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5000/api/github/status', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setGithubStatus({ ...response.data, loading: false });
    } catch (error) {
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <button 
              onClick={githubStatus.connected ? handleGithubDisconnect : handleGithubConnect}
            ...
            >
              <div className={`flex items-center justify-center w-12 h-12 rounded-lg mx-auto mb-4 transition-colors ${
                githubStatus.connected 
              ...
              <h4 className="text-sm font-medium text-gray-900">
                {githubStatus.connected ? 'Disconnect GitHub' : 'Connect GitHub'}
              </h4>
